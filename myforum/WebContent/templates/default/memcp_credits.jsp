		...
		<c:choose><c:when test="${operation == 'transfer'}">
			...
			<input type="hidden" name="formHash" value="${jrun:formHash(pageContext.request)}" />
				...
				<table summary="<bean:message key="memcp_credits_transfer"/>" cellspacing="0" cellpadding="0" width="100%">
					<tbody>
						<tr><th><label for="password"><bean:message key="passwordf"/></label></th><td><input type="password" size="15" name="password" id="password" /><c:if test="${settings.hide_keyboard!=1}"><input type="button" class="keybord_button" style="margin-left: 2px;" onclick="jrunKeyBoard('password',this);" title="<bean:message key="keyboard_title"/>"></c:if>&nbsp;</td></tr>
			...
			<form id="creditsform" method="post" action="memcp.jsp?action=credits">
				...
				<table summary="<bean:message key="EXC"/>"  cellspacing="0" cellpadding="0" width="100%">
					<tbody>
						<tr><th><label for="password"><bean:message key="passwordf"/></label></th><td><input type="password" size="15" name="password" id="password"/><c:if test="${settings.hide_keyboard!=1}"><input type="button" class="keybord_button" style="margin-left: 4px;" onclick="jrunKeyBoard('password',this);" title="<bean:message key="keyboard_title"/>"></c:if>&nbsp;</td></tr>
