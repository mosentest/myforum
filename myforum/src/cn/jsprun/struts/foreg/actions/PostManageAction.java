...
public class PostManageAction extends BaseAction {
	...
	public ActionForward newthread(ActionMapping mapping, ActionForm form,HttpServletRequest request, HttpServletResponse response) {
		...
		try{
			if(submitCheck(request, "topicsubmit")){
				...
				if(forum.getAllowhtml()==1||htmlon==1){
	...
	public ActionForward reply(ActionMapping mapping, ActionForm form,HttpServletRequest request, HttpServletResponse response) {
		...
		Map<String, String> settings = ForumInit.settings;
		Map extcredits = dataParse.characterParse(settings.get("extcredits"),true);
		Map<Integer, Integer> postcredits = dataParse.characterParse(fileds.getReplycredits(),true);
		Map<Integer, Integer> postCreditsCheckMap = postcredits;
		if(postCreditsCheckMap==null||postCreditsCheckMap.size()<=0){
			Map creditspolicys= dataParse.characterParse(settings.get("creditspolicy"),false);
			postCreditsCheckMap=(Map<Integer,Integer>)creditspolicys.get("reply");
			creditspolicys=null;
		}
		String lowerlimitmessage=checklowerlimit(request,extcredits, postCreditsCheckMap, member, 1);
		if (lowerlimitmessage != null) {
			request.setAttribute("errorInfo", lowerlimitmessage);
			return mapping.findForward("showMessage");
		}
		...
		if (request.getParameter("replysubmit") == null || previewpost!=null) {
			...
			try{
				if(submitCheck(request, "replysubmit")){
					...
					if(!Common.empty(ref)&&ref.indexOf("post.jsp")!=-1&&(forum.getAllowhtml()==1||htmlon!=null)){
						message=Common.clearLineBreaksFI(message);
					}
					if(modnewreplies){
	...
	public ActionForward edit(ActionMapping mapping, ActionForm form,HttpServletRequest request, HttpServletResponse response) {
		...
		try{
			if(submitCheck(request, "editsubmit")){
				...
				if(forums.getAllowhtml()==1||post.getHtmlon()==1){
					message=Common.clearLineBreaksFI(message);
				}
				...
				List<Map<String,String>> accesslist = dataBaseService.executeQuery("select allowpost,allowpostattach from jrun_access where uid='"+uid+"' and fid='"+thread.getFid()+"'");
				Map<String,String> access = accesslist.size()>0?accesslist.get(0):null;
				String status = common(request, response, settings, usergroups, fields, forums, thread.getSpecial(), access, fileupload);
				if(status != null ){
					return mapping.findForward(status);
				}
				boolean allowpostattach = (Boolean) request.getAttribute("allowpostattach");
				if(allowpostattach ){
					String attamessage;
					int length2 = filelist.size()-1;
					for (int i = 0; i < length2; i++) {
						FileItem fileItem = (FileItem)filelist.get(i);
						if(fileItem!=null && fileItem.getSize()>0){
							attamessage = checklowerlimit(request, memberss, fields.getPostattachperm(), "postattach", 1);
							if (attamessage != null) {
								request.setAttribute("errorInfo", attamessage);
								return mapping.findForward("showMessage");
							}
							break;
						}
					}
					attamessage = uploadAttachment(request, pid, thread.getTid(), thread.getFid(),Common.addslashes(post.getMessage()),filelist,forums.getDisablewatermark(),fileupload);
					filelist = null;
					if(!attamessage.equals("")){
						request.setAttribute("resultInfo", attamessage);
						return mapping.findForward("showMessage");
	...
	private String checklowerlimit(HttpServletRequest request,Map extcredits,Map<Integer, Integer> creditsarray, Members member, int coef) {
		String message = null;
		if (creditsarray != null && creditsarray.size() > 0 && member != null) {
	...
	@SuppressWarnings("unchecked")
	private String checklowerlimit(HttpServletRequest request, Members member, String forumfields, String action, int coef) {
		Map<String, String> settings = ForumInit.settings;
		Map extcredits = dataParse.characterParse(settings.get("extcredits"),true);
		Map<Integer, Integer> postcredits = dataParse.characterParse(forumfields,true);
		Map<Integer, Integer> creditsarray = postcredits;
		if(creditsarray==null||creditsarray.size()<=0){
			Map creditspolicys= dataParse.characterParse(settings.get("creditspolicy"),false);
			creditsarray=(Map<Integer,Integer>)creditspolicys.get(action);
			creditspolicys=null;
		}
		String message = null;
		if (creditsarray != null && creditsarray.size() > 0 && member != null) {
